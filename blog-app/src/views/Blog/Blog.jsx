import React, { useEffect, useReducer } from 'react';
import { useLocation, useNavigate } from 'react-router-dom';
import { getBlogById } from '../../strapi';
import {
    fetchReducer,
    initialState,
    GET_DATA_TYPE_LOADING,
    GET_DATA_TYPE_SUCCESS,
    GET_DATA_TYPE_FAILURE
} from '../../reducers/fetchReducer';
import { CardMedia, Container, Typography } from '@mui/material';
import Loader from '../../components/Loader/Loader';

const Blog = () => {
    const location = useLocation();
    const navigate = useNavigate();
    const params = new URLSearchParams(location.search);
    const id = params.get('id');
    if (!id) navigate(`/explore`);
    const [blog, blogDispatch] = useReducer(fetchReducer, initialState);
    useEffect(() => {
        console.log(params);
        blogDispatch({ type: GET_DATA_TYPE_LOADING, payload: true });
        const getBlog = async () => {
            try {
                const blogs = await getBlogById(id);
                console.log(blogs.data?.data);
                blogDispatch({ type: GET_DATA_TYPE_SUCCESS, payload: blogs.data?.data });
            } catch (error) {
                blogDispatch({ type: GET_DATA_TYPE_LOADING, payload: false });
                blogDispatch({ type: GET_DATA_TYPE_FAILURE, payload: 'Error' });
                console.error(error);
            }
        };
        getBlog();
    }, []);
    if (!blog.data) return (<Loader open={blog.loading} onClose={() => {}} />);
    return (
        <Container maxWidth="xl">
            <CardMedia
                component="img"
                height="250"
                image={
                    blog.data.attributes.image_content.data?.attributes.formats.large?.url ||
                    blog.data.attributes.image_content.data?.attributes.formats.medium?.url ||
                    blog.data.attributes.image_content.data?.attributes.formats.small?.url ||
                    blog.data.attributes.image_content.data?.attributes.formats.thumbnail?.url ||
                    'data:image/png;base64,' //'ruta_a_imagen_de_placeholder' // Ruta a una imagen de placeholder o una imagen predeterminada en caso de que ninguna de las URLs esté disponible
                }
                alt={blog.data.attributes.image_content.data?.attributes.name}
            />
            <Container maxWidth="md" sx={{ mt: 2 }}>
                <Typography variant="body2" gutterBottom>
                    {blog.data.attributes.release} - {blog.data.attributes.author}
                </Typography>
            </Container>
            <h1 style={{ textAlign: 'center' }}>{blog.data.attributes.title}</h1>
            <Container maxWidth="md">
                <Typography variant="body1" gutterBottom>
                    {blog.data.attributes.text_content}
                </Typography>
            </Container>
        </Container>
    );
};

export default Blog;
